...
public final class AStarPathFinder extends AbstractNodeCostSearch {
    ...
    protected Optional<IPath> calculate0(long primaryTimeout, long failureTimeout) {
        ...
        int pathingMaxChunkBorderFetch = calcContext.baritone.settings().pathingMaxChunkBorderFetch.value; // grab all settings beforehand so that changing settings during pathing doesn't cause a crash or unpredictable behavior
        double minimumImprovement = calcContext.baritone.settings().minimumImprovementRepropagation.value ? MIN_IMPROVEMENT : 0;
        ...
        while (!openSet.isEmpty() && numEmptyChunk < pathingMaxChunkBorderFetch && !cancelRequested) {
            ...
            if (slowPath) {
                try {
                    Thread.sleep(calcContext.baritone.settings().slowPathTimeDelayMS.value);
